...
**1b. Reconcile:** `state.json` (canonical) + `CHANGELOG.md` (history) + live `get_equity_portfolio_holdings`. Sort every leg into **DONE / PENDING / UNPLANNED**. **Mutual-fund legs NEVER appear in broker holdings — the Groww MCP reads stocks only.** Reconcile `fund` / MF keep-items from the user's ticks & comments and `personal/mutual-funds.md`; if the user reports new MF buys/redemptions, update that file too. **First run ever** (empty history, no fills): everything is PENDING — that's normal, proceed.
...
- **Prices from Groww first:** `get_ltp` (+ `fetch_historical_candle_data` / `get_historical_technical_indicators` as needed) for every holding + universe name, plus the index & VIX. If `pinned.regime_gate.enabled`: also compute the benchmark index vs its 200-DMA (the gate input for STEP 4). If `pinned.portfolio_brakes.enabled`: also compute the brake inputs — total open risk Σ((entry − stop) × qty) across held + pending-buy legs as % of corpus, this week's realized + unrealized P&L, and the book's drawdown from its peak (track the peak in `meta.book_peak_inr`; seed it with corpus on first run). **yfinance = fallback only**; validate symbols, skip delisted/renamed.
...
Comments from 1a are already applied and win. Then, if `pinned.regime_gate.enabled` AND the benchmark index closed below its 200-DMA: **new-name buys are deferred** — EXIT legs still execute in full, but their replacement recommendations and ADDs go to `watchlist` (with levels, noted "regime-gated") instead of becoming buy tasks, and re-promote automatically once the index reclaims its 200-DMA. Exits, trims, stops and user instructions are NEVER gated. (Evidence: docs/BACKTEST.md — below-200-DMA breakout buys were net losers under every exit ruleset tested.)

**Portfolio brakes** (if `pinned.portfolio_brakes.enabled`) — evaluate BEFORE any new buy; brakes never gate exits/trims/stops/user instructions and never auto-sell:
- **Open-risk cap:** total open risk > `open_risk_cap_pct` of corpus → defer new buys to `watchlist` (noted "brake: open-risk") until risk frees up (a fill moving to break-even, an exit, or a stop raised).
- **Weekly loss budget:** this week's realized + unrealized losses > `weekly_loss_budget_pct` of corpus → no new buys until the next ISO week; say so in the report.
- **Circuit breaker:** book down ≥ `circuit_breaker_dd_pct` from `meta.book_peak_inr` → halt ALL new buys AND upsert a **p1 Todoist task due today**: `⚠️ [<tag>] CIRCUIT BREAKER: book −X% — de-risk proposal` listing suggested trims/stop-tightening with numbers. The human decides; nothing is sold automatically.

Then per position:
- **EXIT & SWITCH** if: last close < `stop`; OR targeted news flags a broken thesis (merger / downgrade / SEBI action / adverse event); OR trend broken (close < 50-DMA **and** < 200-DMA, RSI < 45, MACD < 0). **HELD (done) positions → do NOT auto-change; PROPOSE in STEP 7 — and the moment the rule fires, upsert a p1 Todoist task due today: `⚠️ [<tag>] PROPOSED EXIT: <ticker> — <one-line reason>` with the case, levels and replacement candidate in the description (every mode, not just auto — the user must never wait for the next chat to learn a held position broke its rules). Complete the task when resolved. PENDING → apply to state.json.** Replacement = deterministic: highest VCP composite in an un-held theme, **min composite ≥ `pinned.min_switch_composite`**, tie-break RS → liquidity; give buy zone / stop / target. **Anti-whipsaw:** never reverse a switch, or re-switch a name whose `switched_on` is within the last `pinned.anti_whipsaw_days` trading days, unless its stop is hit; stamp `switched_on` on any new switch.
...
1) Preflight result; 2) LIVE or LAST-CLOSE; 3) **Todoist feedback acted on** (each → what you did); 4) regime one-liner (+ gate state: open/closed, if `pinned.regime_gate.enabled`; + brake state: open-risk % / week P&L vs budget / drawdown vs breaker, if `pinned.portfolio_brakes.enabled`); 5) **WHAT CHANGED** (or "no change"); 6) **EXIT/SWITCH proposals awaiting confirmation** (held positions); 7) **today's actions** with 🟢 buy / 🔴 stop / 🎯 sell — **always re-surface any resting/lapsed limit order noted on a pending leg** (day orders die at market close; remind the user to re-place or adjust); 8) Todoist sync result; 9) **Next recommended run** (`meta.next_review` date + why) — also upserted as the dated **"⏰ [<tag>] Run /{{COMMAND_NAME}} next"** Todoist task.
